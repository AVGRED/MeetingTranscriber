...
object Resampler {
    ...
    fun resample48kStereoTo16kMono(input: ByteArray): ByteArray {
        val totalGroups = input.size / BYTES_PER_GROUP // 完整 3 帧组数
        val output = ByteArray(totalGroups * 2) // 每组出 2 bytes (1 mono sample)

        var inIdx = 0
        var outIdx = 0

        // 处理完整的 12-byte 组（3 立体声帧 → 1 单声道采样）
        while (inIdx + BYTES_PER_GROUP <= input.size) {
            // 取第 1 帧的 L 和 R（little-endian int16）
            ...
            outIdx += 2
            inIdx += BYTES_PER_GROUP // 跳过 3 帧 × 4 bytes
        }
